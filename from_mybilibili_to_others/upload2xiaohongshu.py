...
def input_topic_into_el(upload_topic_list):
    ...
    # 排除空字符串
    upload_topic_list = [
        i for i in upload_topic_list if not str(i).replace(' ', '') == '']
    ...
    if len(upload_topic_list) > 0:
        for topic in upload_topic_list:
            # 先点击按钮<话题>
            topic_btn = driver.find_element_by_xpath(
                '//*[@id="topicBtn"]/span')
            topic_btn.click()
            force_sleep(span_wait_sec)  # 为了等待异步加载数据，控制个间隔时间
            # 再在[描述]栏里输入一个话题文字
            upload_desc_input = driver.find_element_by_xpath(
                '//*[@id="post-textarea"]')
...
def edit_cover(driver: Edge, cover_img_file):
    # 点击按钮 <编辑封面>
    edit_cover_btn = driver.find_element_by_xpath(
        '//*[@id="publish-container"]/div/div[2]/div[2]/div[2]/button')
    ...
    if str(cover_img_file).replace(' ', '') == '':
        ...
        cover_bar_li = driver.find_element_by_xpath(
            '//*[@id="cover-modal-0"]/div/div/div[2]/div/div[2]/div[2]/div/div[2]/ul/li[7]')
        cover_bar_li.click()
        force_sleep(2)
    else:
        # 【*】使用来自B站的封面图片
        # 切换页签 <上传封面>
        cover_bar_li = driver.find_element_by_xpath(
            '//*[@id="cover-modal-0"]/div/div/div[2]/div/div[2]/div[1]/div[2]')
        cover_bar_li.click()
        force_sleep(2)
        # 把图片输入到上传栏位中
        upload_cover_input = driver.find_element_by_xpath(
            '//*[@id="cover-modal-0"]/div/div/div[2]/div/div[2]/div[3]/div[2]/input')
        upload_cover_input.send_keys(cover_img_file)
        force_sleep(2)

    # 点击按钮 <确定> (关闭编辑封面弹窗)
    cover_confirm_btn = driver.find_element_by_xpath(
        '//*[@id="cover-modal-0"]/div/div/div[3]/div/button[2]/span')
...
def upload_video_one(driver: Edge, video_file_path, video_config):
    ...
    # [描述栏]
    # [描述] 栏位里只允许最多填写 1000 字（包含话题在内），但为了保险起见，这里特地减少了一定的字数最大值控制
    desc_total_max = 900
    # 话题
    ...
    upload_topic_list = str(video_config['upload_topic']).replace(
        '，', ',').split(',')  # 避免出现中文逗号的错误情况，提前把中文逗号替换成英文逗号
    ...
    upload_title_input = driver.find_element_by_xpath(
        '//*[@id="publish-container"]/div/div[2]/div[2]/div[3]/input')
    ...
    topic_len = len(upload_topic__.replace(' ', '').replace(
        ',', '')) + (len([i for i in upload_topic__.split(',') if not i == '']) * 3)
    ...
    # 点击单选<公开>
    permission_public_radio = driver.find_element_by_xpath(
        '//*[@id="publish-container"]/div/div[2]/div[2]/div[8]/div[3]/div[2]/div[1]/label')
    permission_public_radio.click()
    # 点击单选<立即发布>
    pubtime_atonce_radio = driver.find_element_by_xpath(
        '//*[@id="publish-container"]/div/div[2]/div[2]/div[8]/div[5]/div[2]/label[1]')
    ...
    # 点击按钮<发布>
    pub_btn = driver.find_element_by_xpath(
        '//*[@id="publish-container"]/div/div[2]/div[2]/div[9]/button[1]')
...
def upload_videos():
    ...
    video_config_list = read_upload_video_menu(
        video_upload_menu_xiaohongshu_txt_name)
    # 过滤掉已经上传过的视频
    video_config_list = [
        i for i in video_config_list if not read_uploaded_remember_to_judge_isuploaded(i['title'])]
