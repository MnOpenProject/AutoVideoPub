...
from auto_clip_video_byandroid.common.ffmpeg_util import get_duration_from_ffmpeg

def edit_video_action(log_print,driver,force_sleep,video_show_1th_name,video_show_2th_name,elementIdPrefix,video_show_file,editor_tool_audio_import_position=None,editor_tool_paster_position=None,video_show_file_path=None):
    print('\n----------------------- 开始执行针对当前视频的剪辑操作 ---------------------------\n')

    # # 获取视频文件的时长
    # file_path = video_show_file_path
    # video_file_duration = get_duration_from_ffmpeg(file_path)
    # log_print(f'video_file_duration = {video_file_duration}')
    # # 四舍五入并取整数（去除带 .0 的小数点部分）
    # video_file_duration = int(round(float(video_file_duration),0))
    # video_long_sec = video_file_duration + 60 # 当前一个视频的总时长(单位：秒) 多加 60 秒是确保视频能够滑动到最右侧（即视频的末尾）
