...
def video_split_tsfiles_by_ffmpeg(ts_file_root_dir,video_dir,video_file_name,video_format,tsfiles_out_dir,ts_unit_long_s=3):
    ts_file_root_dir = ts_file_root_dir.replace('\\','/')
    # 切片生成m3u8列表命令：
    ...
    tsfiles_out_root_dir = f'{tsfiles_out_dir}/{video_file_name}'
    # 若输出文件夹存在，则清空其中的所有文件
    if os.path.exists(tsfiles_out_root_dir):
        del_files(tsfiles_out_root_dir)
    # 若输出文件夹不存在，则创建
    if not os.path.exists(tsfiles_out_root_dir):
        os.makedirs(tsfiles_out_root_dir)

    # ffmpeg 分解视频时，会先生成该文件，然后再根据该文件逐个分解出 .ts 文件，所以这个文件的输出目录也是控制 .ts 文件的输出目录
    
    out_indexfile_path = f'{tsfiles_out_root_dir}/index.m3u8'
    ...
    # index.m3u8 文件转移位置
    index_dir = f'{ts_file_root_dir}/indextxt'
    # if os.path.exists(index_dir):
    #     os.makedirs(index_dir)
    target_index_path = f'{index_dir}/{video_file_name}_index.m3u8'
    if os.path.exists(target_index_path):
        os.remove(target_index_path)
    shutil.move(out_indexfile_path, target_index_path)
    # # 指令完成后，删除 index.m3u8 文件
    # if os.path.exists(out_indexfile_path):
    ...
    tsfile_name_list = os.listdir(tsfiles_out_root_dir)
    for ts_name in tsfile_name_list:
        ts_file_path_source = f'{tsfiles_out_root_dir}/{ts_name}'
        ts_name_new = ts_name.replace('index','')
        ts_file_path_target = f'{tsfiles_out_root_dir}/{ts_name_new}'
