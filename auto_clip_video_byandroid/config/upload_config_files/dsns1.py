...
# 不同的分集设定不同的精剪数组
def get_paragraph_time_list(episode):
  if episode == '01':
    return ['00:00:06,00:02:28', '00:03:50,00:05:43', '00:08:52,00:11:02', '00:14:32,00:16:03', '00:18:28,00:20:56', '00:22:38,00:25:05']
  return []
...
video_file_config_list = [
  {
    ...
    # 分段剪辑方案：[1] - 精确分段方案; [2] - 粗略分段方案;
    # 程序分段剪辑方案选择逻辑：
    #  当 paragraph_time_list（精确分段参数）为时刻字符串数组时，会默认执行[1] - 精确分段方案；当 paragraph_time_list（精确分段参数）为空数组时，程序会选择 [2] - 粗略分段方案; 去执行分段剪辑；
    # ------------- 精确分段方案参数 -----------------------------------------
    # 精确分段：会根据其中给到的时刻字符串数组，来对视频进行自动剪辑
    # 精确分段参数案例（必须满足该格式要求，否则程序会异常报错）：['00:01:25,00:02:59', '00:03:25,00:04:39']
    # 数组中的时刻字符串是个范围值，即开始时刻到结束时刻，开始时刻和结束时刻用逗号隔开，且时刻字符串必须为 时:分:秒 的3位，每个时间单位必须为2为，不足则补零，比如 00:01:02 表示 1分零2秒
    'paragraph_time_list': get_paragraph_time_list(upload_video_episode_list),
    # ------------- 粗略分段方案参数 start -----------------------------------------
...
video_upload_config_list = [
  {
    ...
    # 类型（自制 或 转载）
    'upload_type': '转载', # 若是转载，则必须填写下面的 video_source 参数，B站视频公约里写了：转载视频必须填写视频来源
    # 转载视频的来源
    'video_source': 'https://so.tv.sohu.com',
    # 标签
